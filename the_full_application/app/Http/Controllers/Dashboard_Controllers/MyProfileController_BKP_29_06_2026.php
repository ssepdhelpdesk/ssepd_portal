...
use App\Models\User;
use App\Models\PiaInstituteMaster;
use Auth;
use File;
use Input;
...
class MyProfileController extends Controller
{
    ...
    {
       $this->middleware('permission:my-profile-list|my-profile-create|my-profile-edit|my-profile-delete', ['only' => ['index','show', 'changePassword', 'check_oldemail']]);
...
public function check_oldemail(Request $request)
{
    $oldemail = trim($request->oldemail);
    ...
    $existUser = User::where('email', $oldemail)
    ->where('status', '1')
    ->where('id', '!=', auth()->id())
    ->exists();

    $existsPiaInstitute = PiaInstituteMaster::where('institute_email_id', $oldemail)
    ->where('status', 'Active')
    ->exists();
    

    if ($existUser || $existsPiaInstitute) {
