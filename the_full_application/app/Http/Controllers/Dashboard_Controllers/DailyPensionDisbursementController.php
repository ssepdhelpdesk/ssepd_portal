...
/*public function daily_pension_disbursement_vs_funds_requirements()
{
    ...
    $funds = PensionFundsRequirement::with(['district', 'block', 'municipality'])
        ->where('for_the_month', $month)
        ->where('status', 1)
        ->get();

    $disbursement = DailyPensionDisbursement::where('for_the_month', $month)
        ->where('status', 1)
        ->get()
        ->keyBy(function ($item) {
            return $item->staff_address_type . '_' . $item->district_id . '_' . $item->block_id . '_' . $item->municipality_id;
        });

    $report = collect();

    foreach ($funds as $f) {
        $key = $f->address_type . '_' . $f->district_id . '_' . $f->block_id . '_' . $f->municipality_id;
        $match = $disbursement[$key] ?? null;

        $district_name = $f->district->district_name ?? 'NA';
        $area_name = $f->address_type == 1
            ? ($f->block->block_name ?? 'NA')
            : ($f->municipality->municipality_name ?? 'NA');

        $report->push([
            'area_type' => $f->address_type == 1 ? 'Block' : 'ULB',
            'area_id' => $f->address_type == 1 ? $f->block_id : $f->municipality_id,
            'district_name' => $district_name,
            'area_name' => $area_name,

            'oap_below_80_requirement' => $f->mbpy_oap_below_80_years,
            'oap_above_80_requirement' => $f->mbpy_oap_above_80_years,
            'widow_pension_requirement' => $f->mbpy_wp,
            'disabled_pension_requirement' => $f->mbpy_dp,
            'sdp_below_80_requirement' => $f->mbpy_sdp_below_80_percent,
            'sdp_above_80_requirement' => $f->mbpy_sdp_above_80_percent,
            'sdoap_requirement' => $f->mbpy_sdoap,
            'clp_requirement' => $f->mbpy_clp,
            'wp_aids_requirement' => $f->mbpy_wp_aids,
            'dp_aids_requirement' => $f->mbpy_dp_aids,
            'unmarried_women_requirement' => $f->mbpy_unmarried_women,
            'orphan_covid_requirement' => $f->mbpy_orphan_due_to_covide,
            'widow_covid_requirement' => $f->mbpy_widow_due_to_covid,
            'divorce_destitute_requirement' => $f->mbpy_divorce_or_destitute,
            'transgender_requirement' => $f->mbpy_transgender,
            'total_benf_requirement' => $f->mbpy_total_beneficiaries,

            'oap_below_80_disbursement' => $match->mbpy_oap_below_80_years ?? 0,
            'oap_above_80_disbursement' => $match->mbpy_oap_above_80_years ?? 0,
            'widow_pension_disbursement' => $match->mbpy_wp ?? 0,
            'disabled_pension_disbursement' => $match->mbpy_dp ?? 0,
            'sdp_below_80_disbursement' => $match->mbpy_sdp_below_80_percent ?? 0,
            'sdp_above_80_disbursement' => $match->mbpy_sdp_above_80_percent ?? 0,
            'sdoap_disbursement' => $match->mbpy_sdoap ?? 0,
            'clp_disbursement' => $match->mbpy_clp ?? 0,
            'wp_aids_disbursement' => $match->mbpy_wp_aids ?? 0,
            'dp_aids_disbursement' => $match->mbpy_dp_aids ?? 0,
            'unmarried_women_disbursement' => $match->mbpy_unmarried_women ?? 0,
            'orphan_covid_disbursement' => $match->mbpy_orphan_due_to_covide ?? 0,
            'widow_covid_disbursement' => $match->mbpy_widow_due_to_covid ?? 0,
            'divorce_destitute_disbursement' => $match->mbpy_divorce_or_destitute ?? 0,
            'transgender_disbursement' => $match->mbpy_transgender ?? 0,
            'total_benf_disbursement' => $match->mbpy_total_beneficiaries ?? 0,
        ]);
    }

    $finalReport = $report->values()->map(fn($item, $index) => array_merge($item, ['sl_no' => $index + 1]));
