...
<div class="contact-widget-area pb-70">
   <div class="container">
      <div class="section-title text-center mb-45">
         <span>REGISTER YOUR NGO/SOCIETY</span>
      ...
      <div class="contact-form">
         ...
         @endif
         <form class="from-prevent-multiple-submits" method="POST" action="{{ route('frontend.ngo.ngo_part_one_store')}}" onsubmit="return Validate()" name="vform" enctype="multipart/form-data">
            @csrf
            @method('post')
            <div class="form-body">
   ...
   document.addEventListener("DOMContentLoaded", function () {
      ...
      function initializeDropdowns() {
         ...
         $('#state-dropdown').on('change', function () {
            ...
            $.ajax({
               url: "{{url('frontend/locations/fetch-districts')}}",
               type: "POST",
               data: {
                  state_id: idState,
                  _token: '{{csrf_token()}}'
         ...
         $('#district-dropdown').on('change', function () {
            ...
            $.ajax({
               url: "{{url('frontend/locations/fetch-municipality')}}",
               type: "POST",
               data: {
         ...
         $('#district-dropdown').on('change', function () {
            ...
            $("#block-dropdown").html('');
            $.ajax({
               url: "{{url('frontend/locations/fetch-block')}}",
               type: "POST",
               data: {
                  district_id: idDistrict,
                  _token: '{{csrf_token()}}'
         ...
         $('#block-dropdown').on('change', function () {
            ...
            $.ajax({
               url: "{{url('frontend/locations/fetch-grampanchayat')}}",
               type: "POST",
               data: {
                  block_id: idBlock,
                  _token: '{{csrf_token()}}'
         ...
         $('#grampanchayat-dropdown').on('change', function () {
            ...
            $.ajax({
               url: "{{url('frontend/locations/fetch-village')}}",
               type: "POST",
               data: {
      ...
      radios.forEach(radio => {
         radio.addEventListener('change', function () {
            ...
            /*fetch(`/ssepd_ngo_working_portal/frontend/get-address-type-content-data/${value}`)*/
            fetch(`{{ url('frontend/get-address-type-content-data') }}/${value}`)
            .then(response => {
               if (!response.ok) {
