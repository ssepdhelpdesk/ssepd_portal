						...
						<table id="example23" class="display nowrap table table-hover table-striped border" cellspacing="0" width="100%">
							<thead class="table-light">
								<tr class="text-center">
									...
									<th>Normal Pensioners<br>({{ $to_the_month }})</th>
									<th>EP Pensioners<br>({{ $to_the_month }})</th>
									<th>Total Beneficiaries<br>({{ $to_the_month }})</th>
									<th>Beneficiaries Difference<br>({{ $to_the_month }} − {{ $from_the_month }})</th>
									<th>Normal Fund<br>({{ $from_the_month }})</th>
									<th>EP Fund<br>({{ $from_the_month }})</th>
									<th>Fund Requirement<br>({{ $from_the_month }})</th>
									<th>Normal Fund<br>({{ $to_the_month }})</th>									
									<th>EP Fund<br>({{ $to_the_month }})</th>
									<th>Fund Requirement<br>({{ $to_the_month }})</th>
									<th>Fund Requirement Difference<br>({{ $to_the_month }} − {{ $from_the_month }})</th>
								</tr>
							</thead>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-end">{{ number_format($data->normal_pensioners_to ?? 0) }}</td>									
									<td class="text-end">{{ number_format($data->ep_pensioners_to ?? 0) }}</td>
									<td class="text-end">{{ number_format($data->beneficiaries_to_month ?? 0) }}</td>
									<td class="text-end fw-bold {{ ($data->difference_of_beneficiaries ?? 0) < 0 ? 'text-danger' : 'text-success' }}">
										{{ number_format($data->difference_of_beneficiaries ?? 0) }}
									</td>
									<td class="text-end">₹{{ number_format($data->normal_fund_from ?? 0, 2) }}</td>
									<td class="text-end">₹{{ number_format($data->ep_fund_from ?? 0, 2) }}</td>
									<td class="text-end">₹{{ number_format($data->funds_from_month ?? 0, 2) }}</td>
									<td class="text-end">₹{{ number_format($data->normal_fund_to ?? 0, 2) }}</td>									
									<td class="text-end">₹{{ number_format($data->ep_fund_to ?? 0, 2) }}</td>
								...
								@empty
								<tr>
									<td colspan="17" class="text-center text-muted">No data found for the selected months.</td>
								</tr>
								@endforelse
							</tbody>
						</table>
