				...
				<div class="card-body">
					...
					<div class="table-responsive m-t-40">
						<table id="example23" class="display table table-hover table-striped border" cellspacing="0" width="100%">
							...
							<tbody>
								@php $i = 1; @endphp
								@if ($SpecialSchoolStaffQuery->isNotEmpty())
								@foreach($SpecialSchoolStaffQuery as $key => $staffDetails)
								<tr>
									<td>{{ $i++ }}</td>
									<td>{{ $staffDetails->district->district_name }}</td>
									<td>{{ $staffDetails->special_school_management_name }}</td>
